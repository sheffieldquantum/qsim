...
from algorithms.cmaes.cmaes_p_concat import run_cmaes_p_concat
import approx
import hchain
...
def evaluate_rows(input_df, start_row, end_row, output_folder_path):
    ...
    for row_id in range(start_row-1, end_row):
        ...
        t = time.time()
        pop, log, hof = run_cmaes_p_concat(v=v, k=k, r=r, seed=seed, generations=gens)
        t = time.time() - t

        best = hof[0]

        suz_err = approx.error(chain,approx.suzuki_solution(k, r),t=2*n)
        op_err = approx.error(chain,approx.r_copies(approx.expand_vals(best), r),t=2*n)
