...
def plot(path):
    ...
    for file_name in os.listdir(path):
        if file_name == 'chain.csv':
           ...
           if 'Grouped k=2' in file_name:
                plt.annotate('k=2', (list(df['gate count'])[0], list(df['error'])[0]), fontsize=16,
                             textcoords='offset pixels', xytext=(30,0))
           elif 'Grouped k=3' in file_name:
                plt.annotate('k=3', (list(df['gate count'])[0], list(df['error'])[0]), fontsize=16,
                             textcoords='offset pixels', xytext=(30,0))
    ...
    plt.title(str(chain) + ', t: 2n', fontsize=18)
    ax.set_xlabel('Total Gate Count', fontsize=20, labelpad=20)
